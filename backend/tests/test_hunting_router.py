...
async def test_hunting_search_returns_normalized_results(async_client):
    token = create_access_token({"sub": "techuser", "role": "tech"})
...
async def test_hunting_search_reports_unsupported_artifact_per_provider(async_client):
    token = create_access_token({"sub": "techuser", "role": "tech"})
    resp = await async_client.post(
        "/api/hunting/search",
